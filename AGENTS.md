...
- **Ask for the CodeRabbit review; it no longer runs automatically** (`.coderabbit.yaml` sets `reviews.auto_review.enabled: false`). Comment `@coderabbitai review` **once, when the diff is finished**. Auto-review fired on every push and each run consumed a per-developer rate-limit slot, so a PR with one commit per round of review fixes burned four slots and spent longer waiting on the limit than on CI. Batch your fixes into one push for the same reason. The limit comment's "Next review available in: N minutes" is not a live clock — it is rewritten only when CodeRabbit runs, so compute the reset as its `updated_at` plus that countdown rather than waiting for the text to change
