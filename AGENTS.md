...
- **Docs state what is true now, not the story of how it got that way.** No "this used to be X", no "renamed from Y", no "found by review on #147", no "moved out of Z". A reader needs the current shape and the constraints that still bind; the history is in git and in the PR that made the change. The one thing that earns a backward glance is a rule whose *reason* is only visible in what went wrong — a compatibility fallback, a version ceiling, a pinned dependency — and even then write the constraint, not the incident. Delete stale narration when you touch a file rather than layering a correction on top of it
- **No docs about docs.** A file explaining what another doc contains, an AGENTS.md section describing the README beside it, a changelog of documentation edits — all of it goes stale silently and none of it tells a reader anything the real doc does not. Write the thing once, in the place that owns it, and link to it
- **Nothing redundant.** Do not document the *absence* of something — "there is no X here, do not add one" is noise about a thing a reader was never looking for. Do not restate what the code, the schema, or the config already says, and do not repeat a rule that a nearer file already carries. If a line would not change what someone does, delete it
