...
## 2. Folder Structure
- `src/app`: `app-shell.tsx` (the `ToolShell` composition) and `router.tsx` (`createAppRouter`). There is deliberately **no** module-level `router` singleton and no query provider — see Core Behaviors. `sections.ts` holds the section rail's data — labels, icons, paths — and `useAdminSections`, which computes each section's `active` flag from the router's current location.
