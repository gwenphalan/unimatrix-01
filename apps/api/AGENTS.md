...
- `src/plugins`: cross-cutting Fastify setup; `index.ts` is the current list and the registration order. `integration-credentials.ts` fetches the configured `SECRETS_INTEGRATION_NAMES` from `@unimatrix/secrets`'s HTTP client at boot, caches them in memory, and decorates `app.integrationCredentials` — see the degradation and conflation notes in §3.
- `src/modules`: route modules grouped by feature (`health`, `content`, `user-data`, `integrations`); `index.ts` registers the active modules. `integrations` exposes `POST /integrations/admin/credentials/refresh`, which re-runs the boot-time fetch on demand.
