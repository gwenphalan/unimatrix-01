...
- **Clerk enforces its own redirect allowlist, and drops a target that misses it without failing.** `AuthProvider` is given `allowedRedirectOrigins` from `ALLOWED_REDIRECT_ORIGINS` in `safe-redirect.ts` — same `ROOT_DOMAIN`, anchored on an origin because Clerk tests `url.origin`. Left unset, Clerk defaults to the current origin plus the eTLD+1 of the frontend API and its subdomains: in production that is `unimatrix-01.dev` and covers every service, but in development it is `accounts.dev`, so a sibling dev server on another loopback port is discarded and a completed sign-in lands on `/` instead of the caller. The only signal is a `warnOnce` in the console. `test/safe-redirect.test.ts` asserts the two allowlists agree case-for-case; a target only one of them accepts is the same silent drop.
