...
The app has two modes, chosen up front: Learn walks new OLL or PLL cases one at a time in a fixed teaching order, so you can mark each as known as you pick it up. Drill draws flashcards at random from whichever cases you've enabled, for fast keyboard-driven recall practice. Both modes track per-case progress in the browser, so learning and training pool selections persist between sessions without an account.
