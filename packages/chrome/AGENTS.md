...
- Composes `@unimatrix/ui` and nothing else. **Never `@unimatrix/auth`.** Both shells take the account control as a `ReactNode` slot, which is what lets a sign-in-free tool like `apps/cube-trainer` import a shell without pulling Clerk into its dependency tree.
