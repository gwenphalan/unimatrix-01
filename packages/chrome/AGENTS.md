...
- `./tool`'s `ToolShell` has two layouts. Supplying a non-empty `sections` moves the wordmark, `accountControl`, and copyright into a collapsible section rail and removes the title bar and footer strip; omitting `sections` (or passing an empty array) keeps today's title-bar-plus-footer layout.
