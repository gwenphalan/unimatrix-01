...
- **Control surfaces**: a control paints `not-in-[.site-panel]:bg-background` and restores its translucent fill with `in-[.site-panel]:`, because a tool surface has no panel behind it while the public site's panels do that separating. Every `dark:` utility in this package is live — all five apps force `.dark` on the root — so a `dark:bg-*` left beside a new background class silently outranks it and the change is a no-op; remove the `dark:bg-*` in the same edit that adds the `in-[.site-panel]:` replacement. A consumer passing its own `bg-*` through `cn` keeps the `in-[.site-panel]:` class alongside it, since tailwind-merge treats them as different modifiers — an override needs the modified form too.
